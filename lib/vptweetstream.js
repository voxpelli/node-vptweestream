...
createStream = function (username, password, stream) {
  ...
  changeStream = function (stream) {
    var request, query, i, length, requestStart = Date.now();
    ...
    if (typeof stream === 'object') {
      ...
      if (query.track || query.follow || query.locations) {
        query = 'filter.json?stall_warnings=true&' + qs.stringify(query);
      } else {
        query = false;
      }
    } else {
    ...
    if (query) {
      ...
      request = newRequest = https.request({
        host : 'stream.twitter.com',
        path : '/1.1/statuses/' + query,
        auth : username + ':' + password,
        headers : {
          'User-Agent' : 'vptweetstream/0.0.2 (http://kodfabrik.se)'
        }
